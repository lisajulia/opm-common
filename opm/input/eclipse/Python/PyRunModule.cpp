...
bool PyRunModule::executeInnerRunFunction(const std::function<void(const std::string&, const std::vector<std::string>&)>& actionx_callback) {
    ...
    try {
        py::object result = this->run_function(this->opm_embedded.attr("current_ecl_state"), this->opm_embedded.attr("current_schedule"), this->opm_embedded.attr("report_step"), this->opm_embedded.attr("current_summary_state"), cpp_callback);
        return result.cast<bool>();
    } catch (const std::exception& e) {
        try {
            py::object result = this->run_function(this->opm_embedded.attr("current_ecl_state"), this->opm_embedded.attr("current_schedule"), this->opm_embedded.attr("report_step"), this->opm_embedded.attr("current_summary_state"));
            return result.cast<bool>();
